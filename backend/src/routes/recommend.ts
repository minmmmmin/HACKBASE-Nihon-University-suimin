...
router.post("/", async (req, res, _next) => {
  ...
  const geminiReply1 = await askGemini(
    ...
    "&keyword=他のパラメータに入れられない情報全般" + 
    "&genre=居酒屋ならG001, 和食ならG004, 洋食ならG005, イタリアンやフレンチならG006, 中華ならG007, 該当しないなら省略、複数欲しいならIDでなく単語でkeywordに" +
    "&free_drink=飲み放題が必要なら1" +
    "&free_food=食べ放題が必要なら1" +
    "&lunch:ランチ指定あるいは今が昼で時間指定なしなら1"
  );
  ...
  const geminiReply2 = await askGemini(
    ...
    '{ conditions: {budgetLevel: "low, medium, high, anyのいずれか", excludedGenres: ["除外するジャンル1", "除外するジャンル2(個数は任せます)"], preferredGenres: ["選択したいジャンル1", "選択したいジャンル2(個数は任せます)"], preferredAtmosphere: ["雰囲気1", "雰囲気2(個数は任せます)"], maxWalkingMinutes: number | null}, shops:[{id: "店1ID", name: "店1名前", genre: "店1ジャンル", budget: "店1予算", access: "店1予算", reason: "あなたが店1を選んだ理由(50文字程度)", distanceMeters: number /* 店1の現在地からの距離(メートル) */ } ] }'
  );
